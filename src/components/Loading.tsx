...
// Card component (simplified; replace with your actual Card if available)
const Card = ({ src, alt, height, width, className }: { src: string; alt: string; height: number; width: number; className?: string }) => (
        ...
        >
          {["/cards/1h.svg", "/cards/1s.svg", "/cards/1d.svg"].map((card, i) => ( // Assuming card image paths
            <motion.div
              key={i}
              variants={{
                hidden: { scale: 0.8, y: 40, rotateY: 0 },
                show: { scale: 1, y: 0, rotateY: 0 },
              }}
              animate={{
                rotateY: [0, 180, 360], // Flip animation
              }}
              transition={{
                rotateY: { duration: 1.5, repeat: Infinity, ease: "easeInOut", delay: i * 0.3 },
              }}
